...
import { PAST_PURCHASES, AVAILABLE_STORES } from '@/data/mockData';
        ...
        <div className="p-6 pb-24 space-y-6">
            ...
            <div className="space-y-4">
                {PAST_PURCHASES.map((purchase, index) => {
                    const store = AVAILABLE_STORES.find(s => s.name === purchase.storeName);
                        ...
                        >
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-full bg-white border border-slate-100 flex items-center justify-center p-2 overflow-hidden shadow-sm">
                                        {store?.logoUrl ? (
                                            <img src={store.logoUrl} alt={purchase.storeName} className="w-full h-full object-contain" />
                                        ) : (
                                            <span className="text-brand-600 font-bold text-lg">{purchase.storeName[0]}</span>
                                        )}
                                    </div>
                                    <div>
                                        <h3 className="font-semibold text-slate-900 uppercase tracking-wide text-sm">{purchase.storeName}</h3>
